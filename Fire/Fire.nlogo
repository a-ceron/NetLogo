...
to setup
  ...
  ;; Para cada patche con valor menor al valor de la densidad será puesto en color verde
  ask patches with [(random-float 100) < density]
    [ set pcolor green ]
  ;; make a column of burning trees
  ifelse Line
      [ ask patches with [pxcor = min-pxcor] [ ignite ] ]
      [ ask n-of Points patches [ ignite ]  ]
  ;;ask patches with [( random-float 1000 ) < point]
  ;; set tree counts
